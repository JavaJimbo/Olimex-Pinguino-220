 ...
 * 2-28-17: Added PIDcontrol() but haven't tested it yet.
 * 
 ****************************************************************************************/
// #define USE_USB
// #define USE_PWM
...
#define PWM_MAX 4095
#define MAXSUM 150  
short errorIntegrator[MAXSUM];

#define MAXDER 15   
short errorDerivative[MAXDER];

short kP = 5000, kI = 12, kD = 4000;
...
#define PCA9685_ADDRESS 0b11000000   
    ...
    if (!initializePCA9685(PCA9685_ADDRESS)) printf("\rWrite error #1");
        ...
        printf("\rSetting M2 to 1600:");        
        
        setPWM(PCA9685_ADDRESS, RIGHTFRONTPWM, 0);     
        setPWM(PCA9685_ADDRESS, LEFTFRONTPWM, 0);
        setPWM(PCA9685_ADDRESS, LEFTREARPWM, 0);
        setPWM(PCA9685_ADDRESS, RIGHTREARPWM, 0);   
    }
    
    while (1) {
        if (TimerCounter > 200) {
            TimerCounter = 0;
            printf(">#%d: RF: %d, LR: %d, RR: %d, LF: %d!\r", counter++, (int) RIGHTFRONTPENC, (int) RIGHTREARENC, (int) LEFTREARENC, (int) LEFTFRONTPENC);
        }
        DelayMs(2);
    }
}
...
short PIDcontrol(short error){
    ...
    sumError = 0;
    for (k = 0; k < MAXSUM; k++) 
        sumError = sumError + (short) errorIntegrator[k];
    
    errorDerivative[i] = error;
    i++; if (i >= MAXDER) i = 0;        
    pastError = (long) errorDerivative[i];
    
    diffError = lngError - pastError;
    
    PCorr = lngError * (long) kP;
    ICorr = sumError * (long) kI;
    DCorr = diffError * (long) kD;

    PIDcorrection = PCorr + ICorr + DCorr;
    PIDcorrection = PIDcorrection / DIVIDER; 
    if (PIDcorrection > PWM_MAX) PIDcorrection = PWM_MAX;
    if (PIDcorrection < -PWM_MAX) PIDcorrection = -PWM_MAX;
    PWMout = (short) PIDcorrection;
